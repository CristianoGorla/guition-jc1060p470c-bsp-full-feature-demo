...
esp_err_t lvgl_demo_music(void)
{
#if LVGL_DEMOS_AVAILABLE && defined(LV_USE_DEMO_MUSIC)
    ...
    ESP_LOGI(TAG, "Music demo started");
    return ESP_OK;
#else
    ESP_LOGW(TAG, "LVGL music demo not available (lv_demos not included)");
    ESP_LOGW(TAG, "Falling back to simple UI demo");
    return lvgl_demo_simple();
#endif
}
...
#elif defined(CONFIG_LVGL_DEMO_MUSIC)
    ESP_LOGI(TAG, "Config: LVGL Music Demo");
    return lvgl_demo_music();
#else
