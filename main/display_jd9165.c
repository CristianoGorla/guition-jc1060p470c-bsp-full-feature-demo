...
{
    // 1. Accensione Backlight su GPIO 23 (LCD_PWM schema Guition) [1]
    gpio_set_direction(23, GPIO_MODE_OUTPUT);
    gpio_set_level(23, 1);

    // 2. Configurazione Bus MIPI DSI (2 lane obbligatorie per Guition) [2]
    esp_lcd_dsi_bus_handle_t mipi_dsi_bus;
    esp_lcd_dsi_bus_config_t bus_config = {
        .bus_id = 0,
        .num_data_lanes = 2,
        .phy_clk_src = MIPI_DSI_PHY_CLK_SRC_DEFAULT,
        .lane_bit_rate_mbps = 800, // Banda calcolata per 1024x600 [3]
    };
    ESP_ERROR_CHECK(esp_lcd_new_dsi_bus(&bus_config, &mipi_dsi_bus));

    // 3. FIX: Uso di DBI over DSI per il Command Mode (v5.5 API)
    esp_lcd_panel_io_handle_t io_handle = NULL;
    esp_lcd_dbi_io_config_t dbi_config = {
        .virtual_channel = 0,
        .lcd_cmd_bits = 8,   // DCS Command bits
        .lcd_param_bits = 8, // DCS Parameter bits
    };
    ...
    esp_lcd_panel_dev_config_t panel_dev_config = {
        .reset_gpio_num = -1, // Reset non mappato o gestito via SW
    ...
    ESP_LOGI(TAG, "Display JD9165 inizializzato con API DBI-over-DSI.");
}
